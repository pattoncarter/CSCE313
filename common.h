...
void send_raw_ip_packet(ipheader *ip)
{
    ...
    inet_ntop(AF_INET, &ip->iph_sourceip, buf1, sizeof(buf1));
    inet_ntop(AF_INET, &ip->iph_destip, buf2, sizeof(buf2));
    printf("%s --> %s\n", buf1, buf2);
    // This will be used by both the server and spoofer
    // error checking for sendto
    if(sendto(sockfd, ip, ntohs(ip->iph_len), 0, (sockaddr*)&dest_info, sizeof dest_info) < 0){
